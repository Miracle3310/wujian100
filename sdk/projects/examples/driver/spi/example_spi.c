...
ElementType data_spi_singel[NBYTE+1]={0};
...
//		printf("send spi\r\n");
		...
		for(i=0;i<=NBYTE;i++){
			data_spi_singel[i]=0xFF;
		}

		csi_spi_ss_control(handle, SPI_SS_ACTIVE);
		mdelay(5);
		
		ret = csi_spi_send(spi_t, data_spi_singel, NBYTE+1);
		printf("%x\n\r",data_spi_singel[NBYTE]);
		mdelay(5);
		
		data_spi_singel[NBYTE]=0xFE;
        for (j = 0; j < (TOTALBYTE / NBYTE); j++)
        {
//			printf("spi send %d\r\n",j);
			
			memcpy(data_spi_singel, data_test+j*NBYTE, sizeof(ElementType)*NBYTE);
			
//			print_data(data_spi_singel,NBYTE+1);
            ret = csi_spi_send(spi_t, data_spi_singel, NBYTE+1);
			mdelay(6);
//			print_data(data_test, NBYTE);
			if(ret<0){
